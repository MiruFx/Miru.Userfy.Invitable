...
public abstract class StubExport
{
    ...
    protected void ExportDir(string dir, Func<string, string> tokens = null)
    {
        foreach (var file in Directory.EnumerateFiles(dir, "*.*", SearchOption.AllDirectories))
        {
            ExportFile(file, tokens: tokens);
        }
    }
    ...
    protected void ExportFile(
            ...
            Func<string, string> tokens = null)
        {
            ...
            if (Params.SolutionMap)
                NewSolutionFiles.Add(destinationFile ?? Path.GetRelativePath(Params.BasedAppDir, file), stubFileName);
        }
